...
async function main() {
  ...
  try {
    console.log("=== INSPECTING RLS FOR LIBRARY TABLES ===");
    
    const rls = await sql`
      SELECT tablename, rowsecurity
      FROM pg_tables
      WHERE schemaname = 'public' AND tablename IN ('library_books', 'library_issues')
    `;
    ...
    const policies = await sql`
      ...
      FROM pg_policies
      WHERE tablename IN ('library_books', 'library_issues')
    `;
