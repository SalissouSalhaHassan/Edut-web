...
function MetaLines({ cfg, align = "left" }: { cfg: DocumentHeaderConfig; align?: "left" | "right" | "center" }) {
  ...
  return (
    <div dir="ltr" lang="fr" className={`space-y-0.5 ${alignClass} text-[11px] font-bold leading-tight text-slate-900 print:text-black french-text`}>
...
function ArabicMetaLines({ cfg }: { cfg: DocumentHeaderConfig }) {
  ...
  return (
    <div
      dir="rtl"
      lang="ar"
      className="arabic-text space-y-1 text-right text-[12px] font-bold leading-relaxed text-slate-900 print:text-black"
    >
      {lines.map((line, index) => (
        <p key={`${line}-${index}`} className={line === schoolNameAr ? "text-[13px] font-black text-slate-950 print:text-black" : undefined}>
          {line}
        </p>
      ))}
    </div>
...
export default function OfficialDocumentHeader({
  ...
  const metaFooter = (printDate || operatorName || qrData) && (
    <div className="mt-2 flex justify-between items-center border-t border-slate-200 pt-2 text-[10px] text-slate-600 font-bold">
  ...
  const sigArea = showSignatureArea && (
    <div className="mt-8 grid grid-cols-2 gap-8 print:mt-6">
      <div className="rounded-2xl border border-dashed border-slate-300 p-4 text-center print:border-black print:border">
        <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Cachet de l'établissement</p>
        <div className="h-16 flex items-center justify-center text-[10px] italic text-slate-400">Emplacement cachet</div>
      </div>
      <div className="rounded-2xl border border-dashed border-slate-300 p-4 text-center print:border-black print:border">
        <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Signature autorisée</p>
        <div className="h-16 flex items-center justify-center text-[10px] italic text-slate-400">Emplacement signature</div>
      </div>
    </div>
  );
  ...
  if (cfg.style === "bilingual_center_logo") {
    return (
      <header className={`official-document-header relative border-b-2 border-slate-900 pb-3 print:border-black text-slate-900 ${className}`}>
            ...
            {qrData && (
              <div className="flex flex-col items-center gap-0.5 bg-white p-1 rounded-lg border border-slate-200 shadow-sm print:shadow-none">
                <img
                  ...
                  className="w-10 h-10"
                />
                <span className="text-[6px] font-black uppercase tracking-widest text-slate-500">VÉRIFICATION</span>
              </div>
            )}
          </div>
        ...
        {title && <h1 className="mt-3 text-center text-xl font-black uppercase tracking-wide text-slate-900 print:text-black">{title}</h1>}
        {metaFooter}
        {sigArea}
      </header>
    );
  ...
  if (cfg.style === "university_formal") {
    return (
      <header className={`official-document-header relative text-center text-slate-900 ${className}`}>
        <div className="grid grid-cols-[100px_1fr_100px] items-center gap-4">
          ...
          <div className="space-y-1">
            ...
            <div className="border-y py-1 border-slate-200" style={{ borderColor: secondary }}>
              <p className="text-sm font-black text-slate-900 print:text-black">{cfg.service || "Service de la Scolarité"}</p>
            </div>
            <p className="text-sm font-bold text-slate-700 print:text-black">{[cfg.bp && `BP : ${cfg.bp}`, cfg.address, cfg.phone && `Tél. ${cfg.phone}`].filter(Boolean).join(", ")}</p>
  ...
  if (cfg.style === "minimal_administrative") {
    return (
      <header className={`official-document-header relative border-b border-slate-300 pb-3 text-slate-900 ${className}`}>
        <div className="flex items-center justify-between gap-6">
          <div>
            <h2 className="text-xl font-black uppercase text-slate-900" style={{ color: primary }}>{cfg.schoolName}</h2>
            <MetaLines cfg={cfg} />
          </div>
          <LogoBox src={cfg.centerLogo || cfg.leftLogo} alt={cfg.schoolName} size={72} />
        </div>
        {title && <h1 className="mt-3 text-center text-lg font-black uppercase text-slate-900 print:text-black">{title}</h1>}
        {metaFooter}
        {sigArea}
      </header>
    );
    ...
    <header className={`official-document-header relative border-b-2 border-slate-900 pb-3 print:border-black text-slate-900 ${className}`}>
      <div className="grid grid-cols-[100px_1fr_100px] items-center gap-6 text-center">
        ...
        <div>
          ...
          {cfg.motto && <p className="mt-1 text-xs font-bold text-slate-600 print:text-slate-600">{cfg.motto}</p>}
        </div>
        <LogoBox src={cfg.rightLogo || cfg.leftLogo || cfg.centerLogo} alt={cfg.schoolName} size={96} />
      </div>
      <div className="mt-3 grid grid-cols-[1fr_220px] gap-4 border-t border-slate-300 pt-3">
        <MetaLines cfg={cfg} />
        <div className="text-center text-sm font-black text-slate-900 print:text-black">
