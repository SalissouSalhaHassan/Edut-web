...
export default function InventoryManager({
  ...
  return (
    <div className="p-8 md:p-10 space-y-8 animate-in fade-in duration-500 bg-slate-50/50 min-h-screen">
      {/* Page Header Banner */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] border border-slate-200/80 shadow-sm">
        <div>
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-indigo-600/10 text-indigo-600 border border-indigo-200 flex items-center justify-center font-black">
              <Package size={24} />
            </div>
            <div>
              <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight">Inventaire & Gestion du Stock</h1>
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            className="h-12 px-6 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-black text-xs uppercase tracking-wider gap-2 shadow-lg shadow-indigo-600/20"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-slate-200/80 shadow-sm flex items-center gap-5">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Articles Total</p>
            <p className="text-2xl font-black text-slate-900">{stats.totalItems}</p>
            <p className="text-[10px] text-slate-400 font-bold mt-0.5">Références enregistrées</p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-slate-200/80 shadow-sm flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-xl font-black text-emerald-600 font-mono">{formatCurrency(stats.totalValue)}</p>
            <p className="text-[10px] text-slate-400 font-bold mt-0.5">Valeur globale du matériel</p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-slate-200/80 shadow-sm flex items-center gap-5">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Matériel Assigné</p>
            <p className="text-2xl font-black text-amber-600">{stats.assignedCount}</p>
            <p className="text-[10px] text-slate-400 font-bold mt-0.5">En possession du personnel</p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-slate-200/80 shadow-sm flex items-center gap-5">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Alerte Stock Bas</p>
            <p className="text-2xl font-black text-rose-600">{stats.lowStockCount}</p>
            <p className="text-[10px] text-slate-400 font-bold mt-0.5">Moins de 3 unités restantes</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className="bg-slate-200/60 p-1.5 rounded-2xl w-fit flex gap-2 border border-slate-200">
          <button
        ...
        {activeTab === "stock" && (
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200/80 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="relative flex-1 w-full max-w-md">
                ...
                <input
                  ...
                  className="w-full h-11 pl-12 pr-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-bold text-slate-900 outline-none focus:border-indigo-500 transition-all"
                />
              </div>
              ...
              <div className="flex items-center gap-3 w-full md:w-auto">
                ...
                <select
                  ...
                  className="h-11 bg-slate-50 border border-slate-200 rounded-2xl px-4 text-xs font-bold text-slate-800 outline-none focus:border-indigo-500"
                >
                    ...
                    filteredItems.map((item) => (
                      <tr key={item.id} className="hover:bg-slate-50/80 transition-all">
                        <td className="px-6 py-5">
                          ...
                          <p className="text-[10px] text-indigo-600 font-mono font-black uppercase tracking-wider">{item.sku || "SKU-N/A"}</p>
                        </td>
                        <td className="px-6 py-5 text-slate-900 font-mono font-bold">
                          {formatCurrency(Number(item.unit_price || item.unitPrice) || 0)}
                        </td>
                        <td className="px-6 py-5 text-center">
                          <span className={`px-4 py-1.5 rounded-full font-black text-sm ${
                            (Number(item.quantity) || 0) === 0 ? "bg-rose-100 text-rose-600" :
                            (Number(item.quantity) || 0) < 3 ? "bg-amber-100 text-amber-600" :
                        ...
                        <td className="px-6 py-5 text-center">
                          <span className={`px-3.5 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider ${
                            item.condition === "Neuf" ? "bg-emerald-100 text-emerald-700" :
                            item.condition === "Endommagé" ? "bg-rose-100 text-rose-700" :
                        ...
                        <td className="px-6 py-5 text-center">
                          <div className="flex justify-center gap-2">
                            <button
                              ...
                              className="w-9 h-9 rounded-xl bg-amber-50 text-amber-600 hover:bg-amber-600 hover:text-white transition-all flex items-center justify-center border border-amber-200"
                            ...
                            <button
                              ...
                              className="w-9 h-9 rounded-xl bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all flex items-center justify-center border border-indigo-200"
                            ...
                            <button
                              ...
                              className="w-9 h-9 rounded-xl bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white transition-all flex items-center justify-center border border-rose-200"
              ...
              <tbody className="divide-y divide-slate-100 text-sm font-bold text-slate-700">
                {assignments.length === 0 ? (
                  ...
                  assignments.map((assign) => (
                    <tr key={assign.id} className="hover:bg-slate-50/80 transition-all">
                      <td className="px-6 py-5 font-black text-slate-900">
                        {assign.item_name || assign.item?.name || "Matériel Consigné"}
                      </td>
                      <td className="px-6 py-5 text-indigo-600 font-extrabold">
                        {assign.employee_nom 
                          ? `${assign.employee_prenom || ''} ${assign.employee_nom}` 
                          : assign.employee?.nom || "Personnel Scolaire"}
                      </td>
                      <td className="px-6 py-5 text-center font-mono font-black text-base">
                        {assign.assigned_qty || assign.assignedQty || 1}
                      </td>
                      <td className="px-6 py-5 text-center">
                        <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${
                          assign.status === "Retourné" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
                        }`}>
      ...
      {showItemModal && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white text-slate-900 rounded-[2.5rem] border border-slate-200 max-w-md w-full p-8 shadow-2xl space-y-6 relative animate-in fade-in zoom-in-95 duration-200">
            <button 
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <input 
                  ...
                  className="w-full h-12 bg-slate-50 border border-slate-200 rounded-2xl px-4 text-sm font-bold text-slate-900 outline-none focus:border-indigo-500"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    className="w-full h-12 bg-slate-50 border border-slate-200 rounded-2xl px-4 text-sm font-bold text-slate-900 outline-none focus:border-indigo-500 uppercase"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    className="w-full h-12 bg-slate-50 border border-slate-200 rounded-2xl px-3 text-xs font-bold text-slate-800 outline-none focus:border-indigo-500"
                  >
                    <option value="Neuf">Neuf</option>
                    <option value="Bon état">Bon état</option>
                    <option value="Moyen">Moyen</option>
            ...
            <div className="pt-4 flex justify-end gap-3 border-t border-slate-100">
              <Button 
                variant="ghost" 
                onClick={() => setShowItemModal(false)}
                className="h-12 px-5 font-bold text-xs text-slate-500 rounded-2xl"
              >
                Annuler
              </Button>
              <Button 
                onClick={handleSaveItem}
                className="h-12 px-6 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl text-xs shadow-lg shadow-indigo-600/20"
              >
                Enregistrer
              </Button>
            </div>
      ...
      {showAssignModal && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white text-slate-900 rounded-[2.5rem] border border-slate-200 max-w-md w-full p-8 shadow-2xl space-y-6 relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <div>
              <h3 className="text-2xl font-black text-slate-900">Affecter du Matériel</h3>
              <p className="text-xs font-bold text-indigo-600 mt-1">{assignForm.itemName}</p>
            </div>
              ...
              <div className="space-y-1">
                ...
                {employees.length > 0 ? (
                  <select
                    ...
                    className="w-full h-12 bg-slate-50 border border-slate-200 rounded-2xl px-4 text-xs font-bold text-slate-900 outline-none focus:border-indigo-500"
                  >
                    {employees.map((emp) => (
            ...
            <div className="pt-4 flex justify-end gap-3 border-t border-slate-100">
              ...
              <Button 
                ...
                className="h-12 px-6 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-2xl text-xs shadow-lg shadow-amber-600/20"
              >
