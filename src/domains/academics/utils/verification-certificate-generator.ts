...
export async function generateVerificationCertificatePDF(
  ...
  // Ribbon Color depending on doc type
  doc.setFillColor(15, 23, 42);
  doc.rect(8, 8, pageWidth - 16, 4.5, "F");

  if (isFinancial) {
    doc.setFillColor(217, 119, 6); // Amber Gold for Financial
  } else if (isBulletin) {
    doc.setFillColor(37, 99, 235); // Blue for School Bulletin
  } else {
    doc.setFillColor(16, 185, 129); // Emerald for Degree / LMD
  }
  ...
  // ─── 5. Section 1: Identification of Holder ──────────────────────────────────
  currentY += qrSize + 5.5;
  ...
  // ─── 6. Section 2: Document Specific Record ──────────────────────────────────
  currentY += 6.5;
  doc.setFillColor(15, 23, 42);
  doc.rect(marginX, currentY, contentWidth, headerBarHeight, "F");
  doc.setFont("helvetica", "bold");
  ...
  // ─── 7. Section 3: Global Standards & Accreditation ──────────────────────────
  currentY += 7;
  doc.setFillColor(15, 23, 42);
  doc.rect(marginX, currentY, contentWidth, headerBarHeight, "F");
  doc.setFont("helvetica", "bold");
  ...
  const fileName = isFinancial
    ...
    : isBulletin
    ? `Attestation_Bulletin_${data.student.matricule.replace(/[^a-zA-Z0-9]/g, "_")}.pdf`
