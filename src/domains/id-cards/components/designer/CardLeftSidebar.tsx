...
import {
  ...
  Stamp, Square, Circle, Minus, Triangle, Plus, Search, Variable,
    ...
    <aside className="w-80 bg-white border-r border-slate-200 flex flex-col h-full select-none z-20 shadow-sm">
      ...
      <div className="p-3 border-b border-slate-100 flex gap-1.5 bg-slate-50/50">
        <button
          ...
          className={`flex-1 py-2 px-2.5 rounded-xl text-[11px] font-black uppercase tracking-wider flex items-center justify-center gap-1.5 transition ${
            activeTab === "library" ? "bg-indigo-600 text-white shadow-md shadow-indigo-100" : "bg-white text-slate-600 hover:bg-slate-100"
          }`}
        >
          <Sparkles size={13} /> العناصر
        </button>
        <button
      ...
      {activeTab === "library" ? (
        <div className="flex-1 overflow-y-auto p-4 space-y-5">
          ...
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2.5">الصور والشعارات</p>
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                ...
                <span className="text-xs font-bold">صورة الطالب</span>
              </button>
              <button
              ...
              >
                ...
                <span className="text-xs font-bold">شعار المدرسة</span>
              </button>
            </div>
          </div>
          ...
          {/* Section: Text */}
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2.5">النصوص والترويسة</p>
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={() => onAddElement("text", { content: "عنوان البطاقة", fontSize: 16, fontWeight: "bold", color: "#312e81" })}
                className="p-3 bg-slate-50 hover:bg-indigo-50/60 border border-slate-200 hover:border-indigo-300 rounded-xl flex flex-col items-center gap-2 text-slate-800 transition group"
              >
                <Type size={18} className="text-indigo-600 group-hover:scale-110 transition-transform" />
                <span className="text-xs font-bold">عنوان بارز</span>
              </button>
              <button
                onClick={() => onAddElement("text", { content: "نص عادي فرعي", fontSize: 11 })}
                className="p-3 bg-slate-50 hover:bg-indigo-50/60 border border-slate-200 hover:border-indigo-300 rounded-xl flex flex-col items-center gap-2 text-slate-800 transition group"
              >
          ...
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2.5">الرموز والأختام الأمنية</p>
            <div className="grid grid-cols-2 gap-2">
              <button
              ...
              >
                ...
                <span className="text-xs font-bold">بارشود Code128</span>
              </button>
              <button
              ...
              >
                ...
                <span className="text-xs font-bold">توقيع الإدارة</span>
              </button>
            </div>
          </div>
          ...
          {/* Section: Shapes */}
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2.5">الأشكال والأطر</p>
            <div className="grid grid-cols-3 gap-2">
              ...
              >
                <Square size={16} />
                <span className="text-[10px] font-bold">مستطيل</span>
              </button>
              <button
              ...
              >
                <Circle size={16} />
                <span className="text-[10px] font-bold">دائرة</span>
              </button>
              <button
              ...
              >
                <Minus size={16} />
                <span className="text-[10px] font-bold">خط فاصل</span>
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-1.5">
            {filteredTokens.map((item) => (
              <button
                ...
                className="w-full p-2.5 bg-slate-50 hover:bg-indigo-50 border border-slate-200 hover:border-indigo-200 rounded-xl flex items-center justify-between text-right transition group"
              >
                <div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">أيقونات تفاعلية</p>
          <div className="grid grid-cols-3 gap-2">
            {[
              { name: "Phone", icon: Phone, label: "هاتف" },
              { name: "Mail", icon: Mail, label: "بريد" },
              { name: "Address", icon: MapPin, label: "عنوان" },
              { name: "School", icon: Building2, label: "مدرسة" },
              { name: "User", icon: User, label: "مستخدم" },
              { name: "Calendar", icon: Calendar, label: "تاريخ" },
            ].map((ic) => {
